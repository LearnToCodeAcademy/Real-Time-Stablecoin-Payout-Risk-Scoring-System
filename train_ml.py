...
def train_token_model(token, model_choice="auto"):
    ...
    for name, m in models.items():
        ...
        try:
            if name in ("xgb", "lgb"):
                # use early stopping with validation set where supported
                if w_train is not None:
                    m.fit(X_train, y_train, sample_weight=w_train, eval_set=[(X_val, y_val)], early_stopping_rounds=20, verbose=False)
                else:
            ...
            # fallback: retry without early stopping/eval_set for boosters
            if name in ("xgb", "lgb"):
                try:
                    print(f"   ? retrying {name} without early stopping...")
                    if w_train is not None:
                        m.fit(X_train, y_train, sample_weight=w_train)
                    else:
                        m.fit(X_train, y_train)
                except Exception as e2:
                    print(f"   [WARN] fallback failed for {name}: {e2}")
                    continue
            else:
                continue
