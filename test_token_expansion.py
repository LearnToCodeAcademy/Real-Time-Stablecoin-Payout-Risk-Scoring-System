...
from wallet_check import detect_token, ALL_TOKENS, SUPPORTED_TOKENS
...
def test_token_list_expansion():
    ...
    # Count by category
    stables = ["USDT", "USDC", "DAI", "BUSD", "USDP", "TUSD", "FRAX", "USDX", "GUSD", "LUSD", "MIM", "USDD", "EURS", "DOLA"]
    ...
    total_expected = len(stables) + len(defi) + len(eth_l2) + len(wrapped) + len(meme)
