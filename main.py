...
CONFIG_V3 = {
    ...
    "min_tiny_tx_count": 10,

    "min_unique_receivers": 10,

    "max_window_days": 7,

    "repeat_small_to_threshold": 5,
...
def run_version_pipeline(version, mode):
    ...
    if version == "v4" and not config.get("SEEDS") and not os.path.exists(POOL_FILES["v4"]):
        ...
        if mode in ["extract", "dual"]:
            ...
            print("[WARN] V4 has no seeds and no V4 pool; generating V4 from existing training-ready data if available.")
        ...
        if mode == "expand":
            ...
            print("[WARN] V4 has no seeds and no V4 pool; skipping expand mode for V4.")
    ...
    if mode == "expand":
        ...
        print(f"[WARN] Unknown mode '{mode}' for {version}")
...
def run_parallel_versions(mode):
    ...
    if not versions:
        ...
        print("[WARN] No enabled versions found for parallel execution")
    ...
    for version, proc in processes:
        ...
        if proc.returncode != 0:
            ...
            print(f"[ERROR] Subprocess failed: {version} returned {proc.returncode}")

        else:

            print(f"[OK] Subprocess completed: {version}")
    ...
    for attempt in range(retry):
        ...
        try:
            ...
            if attempt < retry - 1:
                ...
                print(f"  [ERROR] Timeout after {retry} attempts")
            ...
            if attempt < retry - 1:
                ...
                print(f"  [ERROR] Connection failed after {retry} attempts")
            ...
            if attempt < retry - 1:
                ...
                print(f"  [WARN] Error (attempt {attempt+1}/{retry}): {str(e)[:50]}")
...
def expand_wallets(config, version, use_pool):
    ...
    if not seeds:
        ...
        print(f"[WARN] {version.upper()} NO SEEDS ? EMPTY OUTPUT")
    ...
    try:
        ...
        while frontier and len(visited) < config["MAX_TOTAL_WALLETS"]:
            ...
            if elapsed > max_duration:
                ...
                print(f"\n?? Timeout after {elapsed:.0f}s, stopping expansion")
        ...
        print("\n?? Expansion interrupted by user ? saving pool and returning partial results...")
...
def check_wallet_keywords(address):
    ...
    try:
        ...
        print(f"  [WARN] Scrape error: {str(e)[:40]}")
...
def compute_v3_features(txs, wallet, config, token_filter=None):
    ...
    for tx in txs:
        ...
        try:
            ...
            if value < config["tiny_value_threshold"]:

                tiny_tx_count += 1

                if isinstance(to_addr, str):

                    receiver_small_count[to_addr] = receiver_small_count.get(to_addr, 0) + 1
    ...
    avg_tx_value = total_value / total

    window_days = max(int((max(timestamps) - min(timestamps)) / 86400), 1)

    short_time_window = window_days <= config["max_window_days"]

    repeat_small_to_count = max(receiver_small_count.values()) if receiver_small_count else 0

    no_meaningful_flow = avg_tx_value <= config["tiny_value_threshold"] and total_value <= config["tiny_value_threshold"] * total * 2



    poisoned = int(

        total >= config["min_total_tx"] and

        tiny_tx_count >= config["min_tiny_tx_count"] and

        len(receivers) >= config["min_unique_receivers"] and

        short_time_window and

        repeat_small_to_count >= config["repeat_small_to_threshold"] and
...
def run_v0():
    ...
    for token in TOKENS:
        ...
        if all_rows_by_token[token]:
            ...
            print(f"[OK] V0 {token} DONE ({len(all_rows_by_token[token])})")
...
def run_v1():
    ...
    for token in TOKENS:
        ...
        if all_rows_by_token[token]:
            ...
            print(f"[OK] V1 {token} DONE ({len(all_rows_by_token[token])})")
...
def run_v2():
    ...
    for token in TOKENS:
        ...
        if all_rows_by_token[token]:
            ...
            print(f"[OK] V2 {token} DONE ({len(all_rows_by_token[token])})")
...
def run_v3():
    ...
    for w in wallets:
        ...
        for token in TOKENS:
            ...
            d, s, n, p, tiny_count, unique_receivers, avg_value, window_days, repeat_small, no_flow, short_window = compute_v3_features(txs, w, CONFIG_V3, token_filter=token)
            ...
            all_rows_by_token[token].append({
                ...
                **base,
                ...
                "repeat_small_to_count": repeat_small,

                "no_meaningful_flow": int(no_flow),

                "short_time_window": int(short_window),
...
def extract_features_for_version(config, version, use_pool):
    ...
    if use_pool and pool_file:
        ...
        if wallets:
            ...
            print(f"[WARN] {version.upper()} pool {pool_file} empty, falling back to seeds")
    ...
    if not wallets:
        ...
        print(f"[WARN] {version.upper()} no wallets to extract")
    ...
    for i, w in enumerate(wallets):
        ...
        for token in TOKENS:
            ...
            if version == "v0":
                ...
                d, s, n, p, tiny_count, unique_receivers, avg_value, window_days, repeat_small, no_flow, short_window = compute_v3_features(txs, w, config, token_filter=token)
    ...
    for token in TOKENS:
        ...
        if all_rows_by_token[token]:
            ...
            print(f"[OK] {version.upper()} {token} EXTRACTED ({len(all_rows_by_token[token])}) ? {out_path}")
            ...
            if version == "v3":
                ...
                print("   ? Note: training uses datasets/v3_{token.lower()}.csv; clean/raw files are diagnostics.")
...
def generate_v4_dataset(token, max_v4=V4_MAX):
    ...
    if not os.path.exists(input_path):
        ...
        if os.path.exists(fallback_path):
            ...
            print(f"[WARN] V4 candidate file not found for {token}; falling back to {fallback_path}")

            input_path = fallback_path

        else:

            print(f"[WARN] V4 skip {token}: V4 candidate file not found ({input_path})")
    ...
    if df.empty:
        ...
        print(f"[WARN] V4 skip {token}: candidate dataset empty")
    ...
    if safe_df.empty:
        ...
        print(f"[WARN] V4 {token} produced no high-confidence safe wallets")
    ...
    if len(safe_df) > max_v4:
        ...
        print(f"[WARN] V4 {token} trimmed to {max_v4} rows")



    if safe_df.empty:

        print(f"[WARN] V4 {token} produced no safe wallets")
...
def generate_v0_training_ready(token):
    ...
    if not os.path.exists(input_path):
        ...
        print(f"[WARN] V0 training-ready skip {token}: {input_path} not found")
...
if CLI_VERSION:
    ...
    if mode == "expand":
        ...
        print(f"[WARN] Unknown MODE_RUN '{MODE_RUN}', falling back to 'dual'.")
