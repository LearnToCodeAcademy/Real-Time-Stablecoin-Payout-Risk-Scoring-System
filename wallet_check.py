...
from risk_system.etherscan import EtherscanClient, EtherscanConfigurationError, EtherscanError
from risk_system.reputation import reputation_service
...
def clear_model_cache(token=None):
    """Drop cached artifacts so a newly activated model version is loaded."""
    tokens = [token.upper()] if token else list(MODELS)
    for current in tokens:
        MODELS.pop(current, None)
        SCALERS.pop(current, None)
        FEATURE_COLS.pop(current, None)
...
def apply_rule_based_filters(
    features, prob_malicious, prob_poisoned, token="USDT", token_type="stablecoin", type_thresholds=None
):
    """
    ...
    if features.get("tx_per_day", 0) > max_daily and features.get("avg_tx", 1000) < type_thresholds.get(
    ...
    if (
        features.get("recent_tx", 0) > features.get("avg_tx", 1) * spike_mult
        and features.get("wallet_age_days", 1) > 365
        and features.get("tx_per_min", 0) > 0.1
    ):
...
def classify_decision(prob_malicious, prob_poisoned, conf, features, token="USDT", low_data=False):
    ...
    if prob_poisoned >= poisoned_thresh:
        return (
            "BLOCK",
            f"Poisoned {token} wallet (high confidence - address spoofing) | Rule: {rules.get('risk_profile')}",
        )

    if prob_malicious >= malicious_thresh:
        return (
            "BLOCK",
            f"Malicious {token} wallet (high confidence - {rules.get('description')}) | Threats: {', '.join(rules.get('primary_threats', []))}",
        )

    # Then apply rule-based filters for lower confidence cases (defensive heuristics)
    # Apply type-aware thresholds to rules
    rule_decision, rule_conf, rule_name = apply_rule_based_filters(
        features, prob_malicious, prob_poisoned, token, token_type, type_thresholds
    )
    if rule_decision:
...
def fetch_transactions(address, debug=False, strict=False):
    """
    ...
    try:
        ...
        result = client.token_transfers(address, offset=100, sort="desc")
        if debug:
            print(f"[DEBUG] Etherscan V2 returned {len(result)} token transfers")
            if result:
...
def detect_token(transactions, manual_token=None, debug=False):
    ...
    if detected_token in SUPPORTED_TOKENS:
        ...
        print(
            f"[WARN] Detected token: {detected_token} (count: {count}) - UNSUPPORTED (no model trained){other_str}"
        )
...
def _compute_advanced_features(txs):
    ...
    try:
        ...
        for tx in txs:
            try:
                ...
                if sender and receiver:
                    rows.append(
                        {"amount": amount, "timestamp": timestamp, "sender": sender, "receiver": receiver}
                    )
        ...
        sender_entropy = -np.sum(
            (np.array(list(Counter(senders).values())) / len(senders))
            * np.log2(np.array(list(Counter(senders).values())) / len(senders) + 1e-10)
        )

        unique_receivers = len(set(receivers))
        receiver_entropy = -np.sum(
            (np.array(list(Counter(receivers).values())) / len(receivers))
        ...
        repeat_receiver_ratio = (
            len([c for c in Counter(receivers).values() if c > 3]) / unique_receivers
            if unique_receivers > 0
            else 0
        )
...
def explain_wallet_decision(
    model, scaled_features, features_dict, feature_names, token="USDT", max_display=10
):
    """
...
def generate_features(transactions, wallet_address=None):
    ...
    is_poisoned = int(dust_ratio > 0.5 and similarity_hits > 0 and new_sender_ratio > 0.7)
...
def score_wallet_data(address, manual_token=None, debug=False):
    """Return a structured scoring result for API and stream consumers."""
    started = time.time()
    ...
    try:
        ...
        raise ValueError("Invalid Ethereum address") from exc

    contract_match = next(
        (name for contract, name in TOKEN_CONTRACTS.items() if contract.lower() == normalized),
        None,
    )
    if contract_match:
        return {
            "wallet": normalized,
            "token": contract_match,
            "score": None,
            "decision": "SKIP",
            "reason": f"{contract_match} token contract address is not a user wallet",
            "prob_normal": None,
            "prob_malicious": None,
            "prob_poisoned": None,
            "confidence": None,
            ...
            "data_status": "TOKEN_CONTRACT",
            "threat_intelligence": {"status": "NOT_APPLICABLE", "findings": []},
            "processing_time_ms": (time.time() - started) * 1000,
        }

    threat_intelligence = reputation_service.screen(normalized)
    if threat_intelligence["status"] == "MATCH":
        finding = threat_intelligence["findings"][0]
        label = finding.get("nametag") or ", ".join(finding.get("labels") or []) or "malicious address"
        return {
            "wallet": normalized,
            "token": manual_token.upper() if manual_token else None,
            "score": 1.0,
            "decision": "BLOCK",
            "reason": f"Threat-intelligence match: {label} ({finding.get('source', 'provider')})",
            "prob_normal": None,
            "prob_malicious": None,
            "prob_poisoned": None,
            "confidence": 1.0,
            "features": {},
            "low_data": False,
            "assessment_status": "BLOCKED_BY_REPUTATION",
            "data_status": "REPUTATION_MATCH",
            "threat_intelligence": threat_intelligence,
            "processing_time_ms": (time.time() - started) * 1000,
        }

    try:
        transactions = fetch_transactions(normalized, debug=debug, strict=True)
    except (EtherscanConfigurationError, EtherscanError, requests.RequestException) as exc:
        return {
            "wallet": normalized,
            "token": manual_token.upper() if manual_token else None,
            "score": None,
            "decision": "REVIEW",
            "reason": f"UNSCORABLE: transaction provider unavailable ({exc})",
            "prob_normal": None,
            "prob_malicious": None,
            "prob_poisoned": None,
            "confidence": None,
    ...
    if not transactions:
        return {
            ...
            "score": None,
            "decision": "REVIEW",
            "reason": "UNSCORABLE: provider returned no ERC-20 transfer history; this is not evidence of low risk",
            "prob_normal": None,
            "prob_malicious": None,
            "prob_poisoned": None,
            "confidence": None,
            ...
            "low_data": True,
            "assessment_status": "UNSCORABLE",
            "data_status": "NO_ERC20_HISTORY",
            "threat_intelligence": threat_intelligence,
            "processing_time_ms": (time.time() - started) * 1000,
        }

    token = detect_token(transactions, manual_token=manual_token, debug=debug)
    if not token:
        return {
            "wallet": normalized,
            "token": None,
            "score": None,
            "decision": "REVIEW",
            "reason": "No trained token model matched the wallet's recent transfers",
            "prob_normal": None,
            "prob_malicious": None,
            "prob_poisoned": None,
            "confidence": None,
            ...
            "low_data": True,
            "assessment_status": "UNSCORABLE",
            "data_status": "TOKEN_NOT_DETECTED",
            "threat_intelligence": threat_intelligence,
            "processing_time_ms": (time.time() - started) * 1000,
        }

    features, low_data = generate_features(transactions, normalized)
    if low_data:
        decision = "REVIEW"
        reason = f"Insufficient {token} transaction data for model inference"
        ...
        try:
            load_model_for_token(token)
            frame = align_features_for_token(token, features)
            scaled = SCALERS[token].transform(frame)
            probabilities = get_token_probabilities(MODELS[token], scaled)
            prob_normal, prob_malicious, prob_poisoned = probabilities
            risk_score = max(prob_malicious, prob_poisoned)
            ...
            decision = "REVIEW"
            reason = f"Model unavailable for {token}: {exc}"
            probabilities = (None, None, None)
            confidence = None
            risk_score = None
    ...
    return {
        ...
        "prob_poisoned": prob_poisoned,
        "confidence": confidence,
        "features": features,
        "low_data": low_data,
        "assessment_status": "SCORED" if risk_score is not None else "UNSCORABLE",
        "data_status": "MODEL_SCORED" if risk_score is not None else "INSUFFICIENT_MODEL_DATA",
        "threat_intelligence": threat_intelligence,
        "processing_time_ms": (time.time() - started) * 1000,
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--interactive", "-i", action="store_true", help="Interactive mode (default if no address provided)"
    )
    ...
    if args.address:
        ...
        while True:
            ...
            token_override = (
                input("Token override (press Enter for auto-detect, or type USDT/USDC/etc): ").strip().upper()
                or None
            )
